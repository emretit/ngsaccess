...
BEGIN
    ...
    current_time := p_access_time::time;
    current_day := to_char(p_access_time, 'Day');
    current_day := trim(current_day); -- Remove extra spaces
    
    -- Convert day name to English if needed
    CASE current_day
        WHEN 'Pazartesi' THEN current_day := 'Monday';
        WHEN 'Salı' THEN current_day := 'Tuesday';
        WHEN 'Çarşamba' THEN current_day := 'Wednesday';  
        WHEN 'Perşembe' THEN current_day := 'Thursday';
        WHEN 'Cuma' THEN current_day := 'Friday';
        WHEN 'Cumartesi' THEN current_day := 'Saturday';
        WHEN 'Pazar' THEN current_day := 'Sunday';
        ELSE current_day := to_char(p_access_time, 'FMDay');
    END CASE;
    ...
    LOOP
        ...
        IF rule_record.days IS NOT NULL AND array_length(rule_record.days, 1) > 0 THEN
            IF NOT (current_day = ANY(rule_record.days)) THEN
        ...
        IF rule_record.start_time IS NOT NULL AND rule_record.end_time IS NOT NULL THEN
            IF NOT (current_time BETWEEN rule_record.start_time AND rule_record.end_time) THEN
